...
#include <time.h>
#include "jansson.h"
#include "src/lib/wiringPi/wiringPi.h"
#include "universal.h"
...
static void PowerActorOnHiRequest(PVOID pParam)
{
	...
	ActorFreeSplitMessage(znpSplitMessage);
	ActorSend(PowerActor, responseTopic, responseMessage, NULL, FALSE);
...
static void PowerPublishBlackoutEvent()
{
	...
	ActorSend(PowerActor, topicName, eventMessage, NULL, FALSE);
	json_decref(blackoutJson);
...
static void PowerActorCreate(char* guid, char* psw, char* host, WORD port)
{
	...
	ActorRegisterCallback(PowerActor, ":request/Hi", PowerActorOnHiRequest, CALLBACK_RETAIN);
}
...
static void PowerProcess()
{
	...
	{
		blackoutCount++;
		if (blackoutCount == (10000 / MAIN_LOOP_PERIOD))
		{
			blackoutCount = 0;
			backoutDuration++;
		}
